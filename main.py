...
def exportar_ordenes():
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT id, fecha, cliente, telefono, equipo, falla, presupuesto, ubicacion, estado FROM ordenes ORDER BY id DESC")
    rows = cur.fetchall()
    conn.close()

    output = "ID,Fecha,Cliente,Telefono,Equipo,Falla,Presupuesto,Ubicacion,Estado\n"
    for r in rows:
        output += f'"{r["id"]}","{r["fecha"]}","{r["cliente"]}","{r["telefono"]}","{r["equipo"]}","{r["falla"]}","{r["presupuesto"]}","{r["ubicacion"]}","{r["estado"]}"\n'
