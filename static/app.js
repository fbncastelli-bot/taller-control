...
function armarDatosComprobante(o) {
    ...
    const textoQR = `OT:#${o.id}|Cliente:${o.cliente}|Equipo:${o.equipo}|Estado:${o.estado}|Monto:$${o.presupuesto}`;
    generarQR(textoQR);
}
...
function descargarComprobantePDF() {
    ...
    const area = document.getElementById('area-impresion');
    const bloquePDF = document.getElementById('bloque-comprobante-pdf');
    area.style.display = 'block';

    const opciones = {
        margin:       10,
        filename:     `Comprobante_OT_${o.id}.pdf`,
        image:        { type: 'jpeg', quality: 0.98 },
        html2canvas:  { scale: 2 },
        jsPDF:        { unit: 'mm', format: 'a4', orientation: 'portrait' }
    };

    html2pdf().set(opciones).from(bloquePDF).save().then(() => {
        area.style.display = 'none';
    });
}
...
function renderTablaRepuestos(lista) {
    ...
    lista.forEach(r => {
        ...
        if (cant === 0) badgeClass = 'bg-danger text-white';
        else if (cant <= 2) badgeClass = 'bg-warning text-dark';
        ...
        tr.innerHTML = `
            ...
            <td>${r.ubicacion || ''}</td>
            <td><span class="badge ${badgeClass} fs-6">${cant}</span></td>
            <td class="text-center" onclick="event.stopPropagation()">
                <button onclick="modificarStockDirecto(${r.id}, 1)" class="btn btn-outline-success btn-sm py-0 px-2 fw-bold">+</button>
                <button onclick="modificarStockDirecto(${r.id}, -1)" class="btn btn-outline-warning btn-sm py-0 px-2 fw-bold ms-1">-</button>
            </td>
        `;
        tbody.appendChild(tr);
    });
...
async function modificarStockDirecto(id, delta) {
...
async function modificarStock(delta) {
    ...
    modificarStockDirecto(repSeleccionadoId, delta);
}
